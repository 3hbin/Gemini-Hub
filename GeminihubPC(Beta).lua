...
local MainWidth = IsMobile and 240 or 340
local MainHeight = IsMobile and 460 or 560
local HeaderHeight = IsMobile and 34 or 50
local GridTopOffset = IsMobile and 245 or 300
local CloseButtonHeight = IsMobile and 30 or 35
local CloseButtonBottomGap = IsMobile and 8 or 10
local GridBottomReserve = CloseButtonHeight + CloseButtonBottomGap + (IsMobile and 4 or 10)

local ManagedGuiNames = {
    GeminiHub = true,
    GeminiSearchUI = true,
    GeminiLoading = true,
    ServerBrowser = true,
    ServerBrowserBig = true,
    MobileShiftLock = true,
    ChatAI = true,
    LuaExecutor = true,
    ShirtChanger = true,
    FpsCapChanger = true,
    GamepassBrowser = true,
    EmoteMenu = true,
    FakeGpMenu = true,
    ToolMenu = true,
    AimbotMenu = true,
    TailTrollMenu = true,
    UltimateTrollMenu = true,
    BombTrollMenu = true,
    FreeMorphMenu = true,
    AutoGameSearchMenu = true,
    GeminiSmartphoneV4 = true,
    GeminiNeonGlitchMenu = true,
    GeminiErrorMenu = true,
    GeminiKickPrankUI = true,
    GeminiSettingsMenu = true,
    GeminiKeySystem = true,
    GeminiLinkServerUI = true,
    GeminiVirtualKeyboard = true,
    HomeGui = true,
    GeminiSaveSettingsUI = true,
}

local function isManagedGuiObject(obj)
    local current = obj
    while current do
        if current:IsA("ScreenGui") then
            return ManagedGuiNames[current.Name] == true
        end
...
local function styleTextButton(btn, maxTextSize)
    if not btn or not btn:IsA("TextButton") then return end
    if btn:GetAttribute("GeminiSkipTextStyle") then return end
    ...
    local textLimit = btn:FindFirstChild("GeminiTextLimit") or Instance.new("UITextSizeConstraint", btn)
    textLimit.Name = "GeminiTextLimit"
    textLimit.MinTextSize = IsMobile and 7 or 8
    textLimit.MaxTextSize = maxTextSize or math.max(btn.TextSize, IsMobile and 10 or 12)

    if btn.Text ~= "" and not btn:FindFirstChild("GeminiButtonPadding") then
        local padding = Instance.new("UIPadding", btn)
        padding.Name = "GeminiButtonPadding"
...
local function watchButtonStyle(root)
    root.DescendantAdded:Connect(function(obj)
        if obj:IsA("TextButton") then
            task.defer(function()
                if obj.Parent and isManagedGuiObject(obj) then
                    styleTextButton(obj)
                end
            end)
        end
...
watchButtonStyle(game.CoreGui)
pcall(function() watchButtonStyle(LocalPlayer:WaitForChild("PlayerGui")) end)
...
LockOverlay.Size = UDim2.new(1, 0, 1, -HeaderHeight)
LockOverlay.Position = UDim2.new(0, 0, 0, HeaderHeight)
...
local LockBtnSize = IsMobile and 28 or 35
LockBtn.Size = UDim2.new(0, LockBtnSize, 0, LockBtnSize)
LockBtn.Position = UDim2.new(1, -(LockBtnSize + 10), 0.5, -LockBtnSize / 2)
...
TopFrame.Position = UDim2.new(0, 0, 0, HeaderHeight + (IsMobile and 5 or 0))
...
GridScrollFrame.Size = UDim2.new(1, -12, 1, -(GridTopOffset + GridBottomReserve))
GridScrollFrame.Position = UDim2.new(0, 6, 0, GridTopOffset)
...
GridScrollFrame.ScrollingDirection = Enum.ScrollingDirection.Y
GridScrollFrame.ClipsDescendants = true

local Grid = Instance.new("UIGridLayout", GridScrollFrame)
Grid.FillDirectionMaxCells = IsMobile and 1 or 2
Grid.CellSize = UDim2.new(IsMobile and 1 or 0.5, IsMobile and -4 or -8, 0, IsMobile and 44 or 54)
Grid.CellPadding = UDim2.new(0, IsMobile and 0 or 10, 0, IsMobile and 8 or 10)
...
local function assignGridOrder(obj)
    NextLayoutOrder = NextLayoutOrder + 1
    obj.LayoutOrder = NextLayoutOrder
end

local function normalizeSearchText(text)
    return string.lower((text or ""):gsub("%b[]", ""):gsub("[\n\r]+", " "))
end

local function registerSearchItem(obj, text)
    obj:SetAttribute("GeminiSearchText", normalizeSearchText(text))
    obj:SetAttribute("GeminiDisplayText", text)
    table.insert(SearchItems, obj)
end

local function setAllSearchItemsVisible()
    for _, obj in ipairs(SearchItems) do
...
local function createToggle(text, callback)
    ...
    createCorner(Btn, 8)
    styleTextButton(Btn, IsMobile and 11 or 12)
    registerSearchItem(Btn, text)
...
local function createSlider(text, minVal, maxVal, defaultVal, callback)
    ...
    Track.Text = ""
    Track:SetAttribute("GeminiSkipTextStyle", true)
...
createButton("🔍 Tìm Kiếm Chức Năng", Color3.fromRGB(150, 0, 200), function()
    ...
    local searchWidth = IsMobile and 260 or 320
    local searchHeight = IsMobile and 220 or 220
    local Frame = Instance.new("Frame", ScreenGui)
    Frame.Size = UDim2.new(0, searchWidth, 0, searchHeight)
    Frame.Position = UDim2.new(0.5, -searchWidth / 2, 0.5, -searchHeight / 2)
    ...
    InputBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        -- Quét tất cả item trong menu, gồm nút, toggle và slider.
        for _, obj in ipairs(SearchItems) do
            if obj and obj.Parent then
                local searchText = obj:GetAttribute("GeminiSearchText") or ""
                local displayText = obj:GetAttribute("GeminiDisplayText") or searchText
                local isMatch = keyword == "" or string.find(searchText, keyword, 1, true) ~= nil

                obj.Visible = isMatch
                if keyword ~= "" and isMatch then
                    table.insert(matchList, displayText)
                end
            end
        end
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        -- Hiển thị lại toàn bộ item sau khi đóng tìm kiếm.
        setAllSearchItemsVisible()
        ScreenGui:Destroy()
    end)
end)
...
createButton("⚙️ Lua Code", Color3.fromRGB(80, 100, 50), function()
    ...
    CodeInputBox.Size = UDim2.new(1, -10, 1, -135)
    ...
    ResultBox.Size = UDim2.new(1, -10, 0, 36)
    ResultBox.Position = UDim2.new(0, 5, 1, -88)
    ...
    ExecuteBtn.Position = UDim2.new(0, 5, 1, -43)
    ...
    ClearBtn.Position = UDim2.new(0.5, 3, 1, -43)
...
createButton("⚠️ Chức Năng Error", Color3.fromRGB(255, 50, 50), function()
    ...
    createErrorBtn("🚪 Giả Lập Bị Admin Kick (Prank)", Color3.fromRGB(120, 20, 50), function()
        ...
        KickGui.Name = "GeminiKickPrankUI"
...
createButton("🔗 Link & Server", Color3.fromRGB(150, 50, 200), function()
    local Player = game:GetService("Players").LocalPlayer
    local PlayerGui = Player:WaitForChild("PlayerGui")
    if PlayerGui:FindFirstChild("GeminiLinkServerUI") then PlayerGui.GeminiLinkServerUI:Destroy() end

    local LinkGui = Instance.new("ScreenGui", PlayerGui)
    LinkGui.Name = "GeminiLinkServerUI"
    LinkGui.ResetOnSpawn = false

    local frameWidth = IsMobile and 240 or 280
    local frameHeight = IsMobile and 210 or 220
    ...
    Header.BackgroundColor3 = Color3.fromRGB(95, 55, 165)
    Header.BorderSizePixel = 0
    createCorner(Header, 12)
    ...
    local Input = Instance.new("TextBox", Body)
    ...
    local JoinBtn = Instance.new("TextButton", Body)
    JoinBtn.Size = UDim2.new(1, 0, 0, 38)
    JoinBtn.Text = "Join Server"
    JoinBtn.BackgroundColor3 = Color3.fromRGB(0, 140, 220)
    JoinBtn.TextColor3 = Color3.new(1, 1, 1)
    JoinBtn.Font = Enum.Font.GothamBold
    JoinBtn.TextSize = IsMobile and 10 or 11
    createCorner(JoinBtn, 7)
    styleTextButton(JoinBtn, IsMobile and 11 or 12)
    ...
    local RestartBtn = Instance.new("TextButton", Body)
    RestartBtn.Size = UDim2.new(1, 0, 0, 38)
    RestartBtn.Text = "Restart Script"
    RestartBtn.BackgroundColor3 = Color3.fromRGB(70, 70, 85)
    RestartBtn.TextColor3 = Color3.new(1, 1, 1)
    RestartBtn.Font = Enum.Font.GothamBold
    RestartBtn.TextSize = IsMobile and 10 or 11
    createCorner(RestartBtn, 7)
    styleTextButton(RestartBtn, IsMobile and 11 or 12)
...
createButton("⌨️ Bàn Phím Ảo", Color3.fromRGB(85, 85, 85), function()
    ...
    if PlayerGui:FindFirstChild("GeminiVirtualKeyboard") then PlayerGui.GeminiVirtualKeyboard:Destroy() end

    local KeyGui = Instance.new("ScreenGui", PlayerGui)
    KeyGui.Name = "GeminiVirtualKeyboard"
    KeyGui.ResetOnSpawn = false

    local frameWidth = IsMobile and 250 or 300
    local MainFrame = Instance.new("Frame", KeyGui)
    MainFrame.Size = UDim2.new(0, frameWidth, 0, 150)
    ...
    local KeyGrid = Instance.new("UIGridLayout", Container)
    KeyGrid.CellSize = UDim2.new(0.333, -8, 0, 38)
    KeyGrid.CellPadding = UDim2.new(0, 8, 0, 8)
...
createButton("🏠 Danh Sách Home", Color3.fromRGB(35, 35, 35), function()
    ...
    Frame.BackgroundColor3 = Color3.fromRGB(25, 25, 25); Frame.Active = true
    ...
    local function RefreshList()
        ...
        for i, data in pairs(_G.HomeList) do
            ...
            styleTextButton(Btn, 11)
            Btn.MouseButton1Click:Connect(function() game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = data.CFrame end)
            
            local Del = Instance.new("TextButton", Holder)
            Del.Size = UDim2.new(0.24, 0, 1, 0)
            ...
            styleTextButton(Del, 11)
            Del.MouseButton1Click:Connect(function() table.remove(_G.HomeList, i); RefreshList() end)
        end
    end
...
-- CLOSE BUTTON
local CloseBtn = Instance.new("TextButton", MainFrame)
CloseBtn.Size = UDim2.new(1, -12, 0, CloseButtonHeight)
CloseBtn.Position = UDim2.new(0, 6, 1, -(CloseButtonHeight + CloseButtonBottomGap))
CloseBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
CloseBtn.TextColor3 = Color3.new(1, 1, 1)
CloseBtn.Font = Enum.Font.GothamBold
...
createCorner(CloseBtn, 8)
styleTextButton(CloseBtn, IsMobile and 11 or 13)
